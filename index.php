		...
		<div class="container">
			<div class="row">
												...
												<form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post" enctype="multipart/form-data" class="form-horizontal" name="novoevento">
													<fieldset>
														...
														<div class="form-group">
															...
															<div class="col-md-4">
																<input type="submit" name="novoevento" class="btn btn-success" value="Create Event" />
															</div>
														</div>
				...
				if (!empty($_POST['novoevento'])) {
					...
					// $courseColor = mysqli_query($conection, "call courseColor('" . $course . "')");
					...
					// stored procedure
					$suc = $conection->query("call insEvent('" . $course . "', '" . $title . "', '" . $importance . "', '" . $color . "', '" . $start . "', '" . $end . "', '" . $note . "')");
					// $ins = $conection->prepare("INSERT INTO events (class_no, title, importance, color, start, end, note) VALUES  (?, ?, ?, ?, ?, ?, ?)");
					// $ins->bind_param("ssissss", $course, $title, $importance, $color, $start, $end, $note);
					// $suc = $ins->execute();
